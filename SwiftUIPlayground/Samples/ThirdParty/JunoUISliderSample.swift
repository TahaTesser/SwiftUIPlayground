...
/// This is copied from https://github.com/christianselig/JunoSlider
...
public struct JunoSlider: View {
  ...
  public init(
    ...
    maxSliderValue: CGFloat,
    baseHeight: CGFloat = 9.0,
    expandedHeight: CGFloat = 20.0,
    label: String,
    editingChanged: ((Bool) -> Void)? = nil
  ) {
    ...
    public var body: some View {
      Text("JunoSlider does not support macOS")
    }
  #else
    public var body: some View {
      ZStack {
        ...
        Capsule()
          ...
          .overlay(alignment: .leading) {
            Capsule()
              ...
              .foregroundStyle(
                Color(white: isGestureActive ? 0.85 : 1.0)
              )
              .frame(
                width: calculateProgressWidth(),
                height: isGestureActive ? expandedHeight : baseHeight
              )
          }
          .clipShape(
            .capsule
          )  // Best attempt at fixing a bug https://twitter.com/ChristianSelig/status/1757139789457829902
        ...
        DragGesture(minimumDistance: 0.0)
          .onChanged { value in
            ...
            let newPercentage = min(
              1.0,
              max(0.0, initialPercentage + percentagePointsIncreased)
            )
      ...
      .accessibilityRepresentation {
        Slider(
          value: $sliderValue, in: 0.0...maxSliderValue,
          label: {
            Text(label)
          },
